(function(){
  ...
  function numberHtml(field){if((cfg.connections||[]).includes(field.key)){return`<span class="num-combo" data-num-combo="${field.key}" style="position:relative;display:inline-flex;flex:1"><input id="f-${field.key}" data-key="${field.key}" type="number" inputmode="decimal" step="1" min="0" value="${field.default}" style="flex:1"><button class="num-toggle" type="button" tabindex="-1" aria-label="список" style="margin-left:-22px;width:20px;background:none;border:none;cursor:pointer">▼</button><div class="offset-menu num-menu" hidden style="max-height:230px;overflow-y:auto;overflow-x:hidden">${standardSizes.map(v=>`<button type="button" data-num-value="${v}">${v}</button>`).join("")}</div></span>`;}return`<input id="f-${field.key}" data-key="${field.key}" type="number" inputmode="decimal" step="1" min="0" value="${field.default}" list="dl-${field.key}"><datalist id="dl-${field.key}">${standardSizes.map(v=>`<option value="${v}"></option>`).join("")}</datalist>`}
  ...
  function bind(){
    ...
    document.querySelectorAll(".num-combo").forEach(combo=>{const input=combo.querySelector("[data-key]");const menu=combo.querySelector(".num-menu");combo.querySelector(".num-toggle").addEventListener("click",e=>{e.preventDefault();document.querySelectorAll(".offset-menu").forEach(o=>{if(o!==menu)o.hidden=true});menu.hidden=!menu.hidden;});menu.addEventListener("pointerdown",e=>e.preventDefault());menu.addEventListener("click",e=>{const b=e.target.closest("[data-num-value]");if(!b)return;input.value=b.dataset.numValue;state[input.dataset.key]=input.value;menu.hidden=true;update();});});
  ...
  function calculate(v){
    ...
    switch(cfg.formula){
      ...
      case"roundElbow":{const ang=v.Angle,tbl=ELBOW_DATA[ang];if(!tbl){area=0;note=t.notReady;break;}const Dn=v.D,Dstd=elbowNearestD(ang,Dn),p=tbl[Dstd];const ck=v.conn_D||"minus";const Dr=(ck==="plus")?p[7]:(ck==="nom")?Dstd:p[5];const stdR=p[1];let inner=v.Rax?(v.R-Dstd/2):v.R;let warn="";if(!(inner>0)){inner=stdR;warn=" • R≤0→стандарт";}const eL=(v.eL>0?v.eL:p[2]),eU=(v.eU>0?v.eU:p[3]);area=elbowArea(p,Dr,inner,eL,eU)*q;const cn=(ck==="plus")?"муфта(+)":(ck==="nom")?"номинал":"ниппель(−)";const sn=(Dstd!==Dn)?(" • ближ.D="+Dstd):"";note="Отвод "+ang+"° • "+cn+" D="+Dr+" • R внутр="+Math.round(inner)+" (станд."+stdR+"), ось="+Math.round(inner+Dstd/2)+", наруж="+Math.round(inner+Dstd)+" • стакан "+eL+"/"+eU+" • сегм."+p[4]+sn+warn;break}
