(function(){
  ...
  function numberHtml(field){if((cfg.connections||[]).includes(field.key)){return`<span class="num-combo" data-num-combo="${field.key}" style="position:relative;display:inline-flex;flex:1"><input id="f-${field.key}" data-key="${field.key}" type="number" inputmode="decimal" step="1" min="0" value="${field.default}" style="flex:1"><button class="num-toggle" type="button" tabindex="-1" aria-label="список" style="margin-left:-22px;width:20px;background:none;border:none;cursor:pointer">▼</button><div class="offset-menu num-menu" hidden>${standardSizes.map(v=>`<button type="button" data-num-value="${v}">${v}</button>`).join("")}</div></span>`;}return`<input id="f-${field.key}" data-key="${field.key}" type="number" inputmode="decimal" step="1" min="0" value="${field.default}" list="dl-${field.key}"><datalist id="dl-${field.key}">${standardSizes.map(v=>`<option value="${v}"></option>`).join("")}</datalist>`}
